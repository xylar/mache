...
None remain; resolved questions are recorded in [Decisions](#decisions).

Two items need verification on the pinned Spack during implementation rather
than a decision: that a toolchain name is accepted inside
`packages:all:require`, and the exact set of variables `spack env activate
--sh` emits for a view. Fallbacks for both are given where they apply.
...
1. Chrysalis `gnu`/`openmpi` with polaris through `mache deploy`: the default
   path, Cray-free; compare the captured `activate.sh` with `spack env
   activate --sh` run interactively.
2. Chrysalis `gnu` with E3SM-Unified: exercises the `e3sm` subclass packages
   (`e3sm-scorpio`, `esmf`, `tempestremap`).
3. Chrysalis `gnu` with compass and Albany: exercises the E3SM-only packages
   and the `LD_LIBRARY_PATH` hook.
